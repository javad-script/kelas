    ...
    <>
      ...
      <form action={formAction} className='space-y-4 mt-10'>
        ...
        <Card className='p-2'>
          ...
          <div>
            <label className='block text-sm font-medium mb-1'>دانش آموزان</label>

            {students.map((i: any) => (
              <Accordion key={i.id} type='single' collapsible>
                <AccordionItem value={i.id}>
                  <AccordionTrigger>
                    <span className='text-lg'>{i.name}</span>
                  </AccordionTrigger>
                  <AccordionContent className='divide-y divide-foreground/10'>
                    {i.students.map((j: any) => (
                      <div key={j.id} className='flex items-center justify-between py-2'>
                        <div className='flex gap-4 items-center'>
                          <UserAvatar fallback='' dialog className='size-12' src={j.profileImage} />
                          <label htmlFor={j.id}>
                            <span className='text-base'>{j.fullName}</span>
                          </label>
                        </div>
                        <input
                          onChange={(e) => {
                            if (e.currentTarget.checked) {
                              setSelectedStudents((prev) => [...prev, j.id]);
                            } else {
                              setSelectedStudents((prev) => prev.filter((i) => i !== j.id));
                            }
                          }}
                          className='size-4'
                          value={j.id}
